...
def softmax_loss(x, y):
    ...
    loss = -np.mean(np.log(probabilities[np.arange(logits.shape[0]), y]))

    diff = probabilities
    diff[np.arange(logits.shape[0]), y] -= 1
...
def batchnorm_backward(dout, cache):
    ...
    dvar = np.sum(dxnorm * (x - mean) * (-0.5) * (var + eps) ** (-1.5), axis=0)

    dmean = np.sum(dxnorm * (-1) / np.sqrt(var + eps), axis=0) + dvar * np.sum(-2 * (x - mean), axis=0) / N

    dx = dxnorm / np.sqrt(var + eps) + dvar * 2 * (x - mean) / N + dmean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = gamma * (N * dout - np.sum(dout, axis=0) - x_norm * np.sum(dout * x_norm, axis=0))
    dx /= (N * sigma)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    # var = np.expand_dims(x.var(axis=1), -1)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for f in range(F):
      ...
      for i in range(0, H_):
        for j in range(0, W_):
          out[:, f, i, j] = np.sum(X[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW] * kernel, axis=(1, 2, 3))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout_ = dout.transpose(0, 2, 3, 1).reshape(-1, dout.shape[1])
    dx, dgamma, dbeta = batchnorm_backward(dout_, cache)
    dx = dx.reshape(dout.shape[0], dout.shape[2], dout.shape[3], dout.shape[1]).transpose(0, 3, 1, 2)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    gamma = np.expand_dims(gamma, (0, -1 , -2))
    beta = np.expand_dims(beta, (0, -1, -2))
    ...
    cache = (x.T, x_norm, var, gamma, eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # x_norm = x_norm.reshape(x.shape).T

    # alpha = (dout * gamma).reshape(x.shape)
                                
    # dx = x_norm.shape[0] * alpha - np.sum(alpha, axis=0) - x_norm * np.sum(alpha * x_norm,axis=0)     
    # dx /= (x_norm.shape[0] * sigma)
