...
def compute_saliency_maps(X, y, model):
    ...
    logits = model(X)
    class_logits = logits.gather(1, y.view(-1, 1)).squeeze().sum()
...
def make_fooling_image(X, target_y, model):
    ...
    while True:
        ...
        if pred.item() == target_y:
            break
        target_logits = logits[target_y]
        target_logits.backward()
        # print(X_fooling.grad)
        with torch.no_grad():
            grads = X_fooling.grad
            X_fooling += learning_rate * grads / grads.norm()
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    logits = model(img)
    loss = logits[0, target_y] - l2_reg * img.norm() ** 2
    loss.backward()
    with torch.no_grad():
