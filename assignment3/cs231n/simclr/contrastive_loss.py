...
def sim(z_i, z_j):
    ...
    dot_product = z_i.dot(z_j)
    norm_dot_product = dot_product / (z_i_norm * z_j_norm)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # l(k, k+N)
        denominator = [torch.exp(sim(z_k, z) / tau) for z in out]
        denominator = torch.stack(denominator).sum() - torch.exp(sim(z_k, z_k) / tau)
        l_k_k_N = - ((sim(z_k, z_k_N) / tau) - torch.log(denominator))

        # l(k+N, k)
        denominator = [torch.exp(sim(z_k_N, z) / tau) for z in out]
        denominator = torch.stack(denominator).sum() - torch.exp(sim(z_k_N, z_k_N) / tau)
        l_k_N_k = - ((sim(z_k_N, z_k) / tau) - torch.log(denominator))

        total_loss = total_loss + (l_k_k_N + l_k_N_k)
...
def sim_positive_pairs(out_left, out_right):
    ...
    out_left_norm = out_left / torch.linalg.norm(out_left, dim=1, keepdim=True)
    out_right_norm = out_right / torch.linalg.norm(out_right, dim=1, keepdim=True)
    pos_pairs = torch.diag(out_left_norm @ out_right_norm.T).unsqueeze(-1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = exponential.sum(dim=1).unsqueeze(-1)
    ...
    positive_pair_similarity = sim_matrix[indices, indices+N]
    positive_pair_similarity = torch.cat([positive_pair_similarity, positive_pair_similarity], dim=0)
    ...
    numerator = torch.exp(positive_pair_similarity/tau)
    ...
    log_loss = -(torch.log(numerator) - torch.log(denom))
    loss = log_loss.mean()
