...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    a = b + prev_h@Wh + x@Wx
    next_h = np.tanh(a)
    cache = (x, prev_h, Wx, Wh, b, next_h, D)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, b, next_h, D = cache
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        cache_t = cache[t]
        dx_t, dh0, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:, t, :] + dh0, cache_t)
        dx.insert(0, dx_t)
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    a = x @ Wx + prev_h @ Wh + b
    a_i, a_f, a_o, a_g = np.array_split(a, 4, axis=1)
    ...
    cache = (i, f, o, g, x, Wx, Wh, b, prev_c, prev_h, next_c, next_h, D)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    i, f, o, g, x, Wx, Wh, b, prev_c, prev_h, next_c, next_h, D = cache
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        h_t, c_t, cache_t = lstm_step_forward(x[:, t, :], h_prev, prev_c, Wx, Wh, b)
        h.append(h_t)
        cache.append(cache_t)
        h_prev = h_t
        prev_c = c_t
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        cache_t = cache[t]
        dx_t, dh0, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:, t, :] + dh0, dprev_c, cache_t)
        dx.insert(0, dx_t)
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
