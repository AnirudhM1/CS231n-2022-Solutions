...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption_positional_encoding = self.positional_encoding(caption_embeddings)
        projected_features = self.visual_projection(features).unsqueeze(1)

        tgt_mask = torch.tril(torch.ones(T, T))

        output = self.transformer(caption_positional_encoding, projected_features, tgt_mask)
