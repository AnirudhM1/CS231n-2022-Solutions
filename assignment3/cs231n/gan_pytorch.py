...
def sample_noise(batch_size, dim, seed=None):
    ...
    return -1 + 2 * torch.rand(batch_size, dim)
...
def discriminator_loss(logits_real, logits_fake):
    ...
    true_labels = torch.ones_like(logits_real).squeeze()
    false_labels = torch.zeros_like(logits_fake).squeeze()
...
def generator_loss(logits_fake):
    ...
    true_labels = torch.ones_like(logits_fake).squeeze()
