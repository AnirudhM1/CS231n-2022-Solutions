...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        theta = torch.arange(max_len).unsqueeze(1) * 10000 ** (-torch.arange(0, embed_dim, 2) / embed_dim)
        pe[0, :, 0::2] = torch.sin(theta)
        pe[0, :, 1::2] = torch.cos(theta)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.n_head
        query = self.query(query).view(N, S, H, E//H).transpose(1, 2) # (N, H, S, E/H)
        key = self.key(key).view(N, T, H, E//H).transpose(1, 2) # (N, H, T, E/H)
        value = self.value(value).view(N, T, H, E//H).transpose(1, 2) # (N, H, T, E/H)

        a = torch.matmul(query, key.transpose(2, 3)) / math.sqrt(E//H) # (N, H, S, T)
        if attn_mask is not None:
          a = a.masked_fill(attn_mask==0, -torch.inf)
          
        y = F.softmax(a, dim=-1)
        y = self.attn_drop(y)
        y = torch.matmul(y, value) # (N, H, S, E/H)
